...
class Batch(Base):
    ...
    status = Column(SAEnum(BatchStatus, native_enum=False), default=BatchStatus.PENDING)
...
class Shipment(Base):
    ...
    mode = Column(SAEnum(ShipmentMode, native_enum=False), nullable=False)
    tenant_id = Column(Integer, ForeignKey("tenants.id"), nullable=True, index=True)
    status = Column(SAEnum(ShipmentStatus, native_enum=False), default=ShipmentStatus.CREATED)
...
class ShipmentBatch(Base):
    ...
    item_shipment_status = Column(SAEnum(ItemShipmentStatus, native_enum=False), default=ItemShipmentStatus.PENDING, nullable=True)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
