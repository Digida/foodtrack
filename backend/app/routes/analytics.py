...
async def api_dashboard(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return await get_dashboard_stats(db)


@router.get("/products-by-category")
async def api_products_by_category(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return {"categories": await get_products_by_category(db)}


@router.get("/events-by-type")
async def api_events_by_type(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return {"event_types": await get_events_by_type(db)}


@router.get("/certificates-by-status")
async def api_certificates_by_status(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
