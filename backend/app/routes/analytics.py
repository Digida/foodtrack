...
async def api_products_by_category(db: AsyncSession = Depends(get_db)):
    return {"categories": await get_products_by_category(db)}


@router.get("/events-by-type")
async def api_events_by_type(db: AsyncSession = Depends(get_db)):
    return {"event_types": await get_events_by_type(db)}


@router.get("/certificates-by-status")
async def api_certificates_by_status(db: AsyncSession = Depends(get_db)):
