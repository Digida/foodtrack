...
// ─── EARNING OPPORTUNITIES (public browse) ─────────────────────

const _EARNING_KINDS = {
  pipeline_job: { icon: '\u{1F9D1}\u200D\u{1F33E}', label: 'Member job' },
  courier_job: { icon: '\u{1F69A}', label: 'Courier job' },
  supply: { icon: '\u{1F6D2}', label: 'Supply aggregation' },
};

const _esc = (v) => String(v ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
const _fmtDate = (s) => { if (!s) return ''; const d = new Date(s); return isNaN(d) ? '' : d.toLocaleDateString(); };
const _badge = (text, cls) => `<span class="badge ${cls}">${_esc(text)}</span>`;

Pages.earning = (app) => {
  app.appendChild(UI.publicLayout(async () => {
    ...
    body.innerHTML = `
      <section class="page-header">
        <div class="section-inner">
          ...
          <p>Browse work generated by FoodTrack's sourcing pipelines — member jobs, courier deliveries and supply aggregation campaigns. Browse for free, log in to participate.</p>
        </div>
      </section>
      <section class="section">
        <div class="section-inner" style="max-width:1100px">
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px">
            <button class="btn btn-sm opp-tab active" data-kind="">All</button>
            <button class="btn btn-sm opp-tab" data-kind="pipeline_job">Member jobs</button>
            <button class="btn btn-sm opp-tab" data-kind="courier_job">Courier jobs</button>
            <button class="btn btn-sm opp-tab" data-kind="supply">Supply aggregation</button>
          </div>
          <div id="earning-list"></div>
          <div id="earning-more" style="text-align:center;margin-top:16px;display:none">
            <button class="btn btn-outline" id="earning-load-more">Load more</button>
          </div>
        </div>
      </section>`;

    const list = body.querySelector('#earning-list');
    const moreWrap = body.querySelector('#earning-more');
    let kind = '';
    let page = 1;
    let totalPages = 1;

    const render = (o) => {
      const meta = _EARNING_KINDS[o.type] || { icon: '💼', label: o.type };
      const reg = o.register || {};
      const item = o.item || {};
      let title, desc;
      const chips = [];
      if (item.item_name) chips.push(_badge('🌿 ' + item.item_name, 'badge-secondary'));
      if (o.type === 'courier_job') {
        title = `Courier delivery · ${o.pickup_location || 'pickup'} → ${o.deliver_to_buyer ? 'buyer doorstep' : 'warehouse #' + (o.dropoff_warehouse_id || '?')}`;
        desc = o.tracking_code ? `Tracking ${o.tracking_code}` : (o.courier_name ? `Courier: ${o.courier_name}` : 'Open for courier assignment');
        if (o.quantity) chips.push(_badge(`📦 ${o.quantity} ${o.unit || ''}`, 'badge-secondary'));
        if (o.weight_kg) chips.push(_badge(`⚖️ ${o.weight_kg} kg`, 'badge-secondary'));
        if (o.budget) chips.push(_badge(`💰 ${o.currency || ''} ${o.budget}`, 'badge-success'));
        if (o.pickup_location) chips.push(_badge('📍 ' + o.pickup_location, 'badge-secondary'));
      } else if (o.type === 'supply') {
        title = o.title || (item.item_name ? `Supply ${item.item_name}` : 'Supply aggregation');
        desc = o.register_number ? `Register ${o.register_number}` : '';
        if (o.sourcing_entity_name) desc += (desc ? ' · ' : '') + `by ${o.sourcing_entity_name}`;
        if (o.region) chips.push(_badge('📍 ' + o.region, 'badge-secondary'));
        if (o.target_quantity) chips.push(_badge(`🎯 ${o.target_quantity} ${o.unit || ''}`, 'badge-secondary'));
        if (o.target_price) chips.push(_badge(`💰 ${o.currency || ''} ${o.target_price}/${o.unit || 'unit'}`, 'badge-success'));
      } else {
        title = `${(o.role || 'Member job').replace(/_/g, ' ')} · ${item.item_name || 'food item'}`;
        desc = o.assignee_name ? `Assigned to ${o.assignee_name}` : (reg.register_number ? `Register ${reg.register_number}` : '');
        if (o.assignee_location) chips.push(_badge('📍 ' + o.assignee_location, 'badge-secondary'));
        if (reg.register_number) chips.push(_badge('#' + reg.register_number, 'badge-secondary'));
      }
      if (reg.title && o.type !== 'supply') desc = (desc ? desc + ' · ' : '') + reg.title;

      const card = document.createElement('div');
      card.className = 'card opp-card';
      card.innerHTML = `
        <div class="card-header">
          <div>${_badge(meta.icon + ' ' + meta.label, 'badge-info')} ${_badge(String(o.status || '').replace(/_/g, ' '), 'badge-secondary')}</div>
          <span style="font-size:12px;color:var(--text-light)">${_fmtDate(o.created_at || o.assigned_at || o.posted_at)}</span>
        </div>
        <h3 style="margin:0 0 6px">${_esc(title)}</h3>
        <div style="font-size:13px;color:var(--text-light);margin-bottom:10px">${_esc(desc)}</div>
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px">${chips.join('')}</div>
        <button class="btn btn-primary btn-sm">${loggedIn ? 'Open & participate' : 'Login to participate'}</button>`;
      card.querySelector('button').addEventListener('click', () => {
        if (!loggedIn) { Router.navigate('#login'); return; }
        Router.navigate(reg.id ? `#bulking/${reg.id}` : '#bulking');
      });
      return card;
    };

    const load = async (reset) => {
      if (reset) list.innerHTML = '<div class="spinner" style="margin:40px auto"></div>';
      try {
        const data = await API.get(`/earning/opportunities?page=${page}${kind ? `&kind=${kind}` : ''}`);
        totalPages = data.total_pages;
        if (reset) list.innerHTML = '';
        if (!data.total) {
          list.innerHTML = '<div class="empty-state" style="text-align:center;padding:40px 0"><p>No open opportunities right now — check back soon.</p></div>';
        } else {
          data.opportunities.forEach(o => list.appendChild(render(o)));
        }
        moreWrap.style.display = page < totalPages ? 'block' : 'none';
      } catch (e) {
        list.innerHTML = `<div class="empty-state" style="text-align:center;padding:40px 0"><p>Error loading opportunities: ${_esc(e.message)}</p></div>`;
      }
    };

    body.querySelectorAll('.opp-tab').forEach(btn => {
      btn.addEventListener('click', () => {
        body.querySelectorAll('.opp-tab').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
    ...
    body.querySelector('#earning-load-more').addEventListener('click', () => { page += 1; load(false); });
    load(true);

    _setSEO('Earning Opportunities — FoodTrack', 'Browse pipeline-generated earning opportunities: member jobs, courier deliveries and supply aggregation campaigns.');
    return body;
  }));
};
